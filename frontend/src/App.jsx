...
const TABS = [
  { id: 'quick', label: 'Quick Analysis', icon: 'M13 10V3L4 14h7v7l9-11h-7z' },
  { id: 'advanced', label: 'Advanced', icon: 'M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z' },
...
export default function App() {
  ...
  const syncAnalysis = useAnalysis()
  const asyncAnalysis = useAsyncAnalysis()
  const toast = useToast()

  const isQuick = activeTab === 'quick'
  const analysis = isQuick ? syncAnalysis : asyncAnalysis
  const { results, loading, error } = analysis
  const reset = analysis.reset

  // Show toast on error
  if (error) {
    // We use a ref-like pattern to avoid re-toasting
  }

  return (
    <Layout>
      <div className="grid gap-6">
        {/* Tab navigation — pill tabs with animated indicator */}
        <div className="flex gap-1 p-1.5 rounded-2xl glass-card-strong gradient-border">
          {TABS.map(tab => (
            <button
              key={tab.id}
              onClick={() => { setActiveTab(tab.id); reset() }}
              className={`relative flex-1 py-3 px-4 rounded-xl text-xs font-medium transition-all duration-300 flex items-center justify-center gap-2
                ${activeTab === tab.id
                  ? 'text-white'
        ...
        {/* Dashboard tab with scroll reveal */}
        {activeTab === 'dashboard' && (
          <ContainerScroll
            titleComponent={
        ...
        {activeTab !== 'dashboard' && activeTab !== 'live' && (
          <>
            ...
            <AnimatePresence>
              {loading && isQuick && (
                <AnalysisProgress progress={syncAnalysis.uploadProgress} />
              )}
              {loading && !isQuick && (
                <PipelineVisualization
                  currentStep={asyncAnalysis.currentStep}
            ...
            <AnimatePresence>
              {results && results.success && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
                  ...
                  {!isQuick && results.job_id && (
                    <div className="flex items-center gap-4">
                      <div className="flex-1">
                        <FeedbackPanel jobId={results.job_id} />
                      </div>
